...
export const ShareModal: React.FC<ShareModalProps> = ({ isOpen, onClose, profileName }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-lg rounded-3xl bg-slate-900 border border-slate-800 p-6 sm:p-7 shadow-2xl text-white max-h-[90vh] overflow-y-auto">
        {/* Close button */}
        <button
          type="button"
          onClick={onClose}
        ...
        <div className="text-center space-y-2 mb-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-950/70 border border-emerald-500/40 text-emerald-400 text-xs font-semibold mb-1">
          ...
          <h2 className="text-xl sm:text-2xl font-extrabold font-display text-white">Share {profileName}'s Hub</h2>
        ...
        <div className="mb-4 p-3.5 rounded-2xl bg-amber-950/40 border border-amber-500/40 text-left space-y-1.5">
          <div className="flex items-center gap-2 text-xs font-bold text-amber-300">
            <AlertCircle className="w-4 h-4 shrink-0 text-amber-400" />
            <span>Why users saw "Loading website" previously:</span>
          </div>
          <p className="text-[11px] text-amber-200/90 leading-relaxed">
            You shared the private <strong>Development URL (<code className="bg-amber-900/60 px-1 py-0.5 rounded text-amber-200">ais-dev-...</code>)</strong> from your browser bar. Dev links sleep when your AI Studio tab is closed!
          </p>
        ...
        <div className="space-y-1.5 mb-4">
          ...
          <div className="flex items-center gap-2 p-2 bg-slate-950 border border-slate-800 rounded-xl">
            <input
              ...
              className="flex-1 bg-transparent px-2 text-xs text-slate-200 font-mono focus:outline-none truncate select-all"
            />
            <button
              type="button"
              id="copy-share-url-btn"
              onClick={() => handleCopy(publicShareUrl)}
              className="px-3 py-1.5 rounded-lg bg-blue-600 text-white font-semibold text-xs hover:bg-blue-500 transition-colors flex items-center gap-1 shrink-0 active:scale-95 shadow-xs"
            >
        ...
        <div className="flex justify-center mb-4">
          <div className="p-3 bg-white rounded-2xl border border-slate-700 shadow-md flex items-center justify-center">
            <svg
        ...
        <button
          ...
          className="w-full py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm shadow-md transition-all flex items-center justify-center gap-2 active:scale-98"
        >
